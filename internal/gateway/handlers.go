...
func (g *Gateway) submitHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := g.storage.PutList(ctx, id, []byte(listContent)); err != nil {
		log.Printf("storage PutList error: %v", err)
	...
	if strings.HasPrefix(ct, "application/x-www-form-urlencoded") {
		w.Header().Set("Content-Type", "text/html")
		fmt.Fprintf(w, `<div id="result" hx-ext="sse" sse-connect="/is-sorted/%s/events?format=html" sse-swap="status:innerHTML result:innerHTML" sse-close="close">`+
